...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (isPrivilegedRole(role)) {
      const [paymentsSnap, enrollmentSnap, usersTutorSnap, usersStaffSnap] = await Promise.all([
        adminDb.collection("payments").limit(100).get(),
        adminDb.collection("enrollment_requests").limit(100).get(),
    ...
    if (isStaffRole(role)) {
      const tutorPayments = await docs("payments", "tutorId", decoded.uid, 100);
      ...
      for (const field of ["tutorId", "staffId", "assignedTutorId", "assignedStaffId", "instructorId"]) {
        for (const collectionName of ["individualStudents", "students"]) {
          try {
            const records = await docs(collectionName, field, decoded.uid, 100);
            records.forEach(record => studentMap.set(record.id, record));
          } catch (error) {
            console.warn(`Assigned ${collectionName}/${field} lookup failed:`, error);
          }
        }
      }
      const walletSnap = await adminDb.collection("staffWallets").doc(decoded.uid).get();
      const withdrawalSnap = await adminDb.collection("walletWithdrawals").where("staffId", "==", decoded.uid).limit(25).get();
      return {
        statusCode: 200,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          wallet: walletSnap.exists ? plain(walletSnap.data()) : { availableBalance: 0, reservedBalance: 0, lifetimeEarned: 0 },
    ...
    const payments = role === "parent"
      ? await docs("payments", "parentId", decoded.uid, 100)
      : role === "school"
        ? await docs("payments", "schoolId", schoolId, 100)
        : await docs("payments", "userId", decoded.uid, 100);

    const enrollments = role === "parent"
      ? await docs("enrollment_requests", "parentId", decoded.uid, 100)
      : role === "school"
        ? await docs("enrollment_requests", "schoolId", schoolId, 100)
        : [];

    const students = role === "parent"
      ? [...await docs("individualStudents", "parentId", decoded.uid, 100), ...await docs("students", "parentId", decoded.uid, 100)]
      : role === "school"
        ? [...await docs("individualStudents", "schoolId", schoolId, 100), ...await docs("students", "schoolId", schoolId, 100)]
        : [];
