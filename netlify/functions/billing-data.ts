...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (isStaffRole(role)) {
      ...
      return { statusCode: 200, headers: { "Content-Type": "application/json", "Cache-Control": "no-store" }, body: JSON.stringify({ role, config, payments, students, wallet: walletSnap.exists ? plain(walletSnap.data()) : { availableBalance: 0, reservedBalance: 0, lifetimeEarned: 0 }, withdrawals: withdrawalSnap.docs.map(doc => ({ id: doc.id, ...plain(doc.data()) })) }) };
    }
