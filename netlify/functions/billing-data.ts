...
const safeStudent = (record: any) => { const { accessCodeHash, ...safe } = record || {}; return safe; };
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (isPrivilegedRole(role)) {
      ...
      return { statusCode: 200, headers: { "Content-Type": "application/json", "Cache-Control": "no-store" }, body: JSON.stringify({ role, config, payments: paymentsSnap.docs.map(doc => ({ id: doc.id, ...plain(doc.data()) })), enrollments: enrollmentSnap.docs.map(doc => ({ id: doc.id, ...plain(doc.data()) })), withdrawals: withdrawalsSnap.docs.map(doc => ({ id: doc.id, ...plain(doc.data()) })), staff: Array.from(staffMap.values()).filter(item => isActiveRecord(item)) }) };
    }
    if (isStaffRole(role)) {
      const tutorPayments = await docs("payments", "tutorId", decoded.uid, 100); const studentMap = new Map<string, any>();
      for (const field of ["tutorId", "staffId", "assignedTutorId", "assignedStaffId", "instructorId"]) for (const collectionName of ["individualStudents", "students"]) { try { const records = await docs(collectionName, field, decoded.uid, 100); records.forEach(record => studentMap.set(record.id, safeStudent(record))); } catch (error) { console.warn(`Assigned ${collectionName}/${field} lookup failed:`, error); } }
      ...
      return { statusCode: 200, headers: { "Content-Type": "application/json", "Cache-Control": "no-store" }, body: JSON.stringify({ role, config, payments: tutorPayments, students: Array.from(studentMap.values()), wallet: walletSnap.exists ? plain(walletSnap.data()) : { availableBalance: 0, reservedBalance: 0, lifetimeEarned: 0 }, withdrawals: withdrawalSnap.docs.map(doc => ({ id: doc.id, ...plain(doc.data()) })) }) };
    }
    ...
    return { statusCode: 200, headers: { "Content-Type": "application/json", "Cache-Control": "no-store" }, body: JSON.stringify({ role, config, payments, enrollments, students, notifications: notificationSnap.docs.map(doc => ({ id: doc.id, ...plain(doc.data()) })) }) };
