...
export const handler: Handler = async event => {
  ...
  try {
    ...
    const inquiryId = clean(body.inquiryId, 160);
    if (!inquiryId) return json(400, { error: 'inquiryId is required.' });
    const inquiryRef = adminDb.collection('inquiries').doc(inquiryId);
    const inquirySnap = await inquiryRef.get();
    if (!inquirySnap.exists) return json(404, { error: 'School partnership inquiry not found.' });
    const inquiry = inquirySnap.data() || {};
    if (String(inquiry.type || '').toUpperCase() !== 'SCHOOL_PARTNERSHIP_PROPOSAL') return json(400, { error: 'This inquiry is not a school partnership proposal.' });
    if (String(inquiry.schoolId || '').trim()) return json(409, { error: 'This partnership inquiry has already been onboarded.' });

    const schoolName = clean(inquiry.schoolName || inquiry.name, 160);
    const contactName = clean(inquiry.name || inquiry.contactName, 120);
    const email = clean(inquiry.email, 160).toLowerCase();
    const phone = clean(inquiry.phone, 40);
    const address = clean(inquiry.addressCity || inquiry.address, 300);
    if (!schoolName || !contactName || !email) return json(400, { error: 'The inquiry is missing school name, contact name, or email.' });
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return json(400, { error: 'The partnership contact email is invalid.' });
    const duplicate = await adminDb.collection('schools').where('name', '==', schoolName).limit(1).get();
    if (!duplicate.empty) return json(409, { error: 'A school with this name already exists. Review the existing school before approving this inquiry.' });

    const authUser = await adminAuth.createUser({ email, displayName: contactName, emailVerified: false });
    ...
    try {
      await adminDb.runTransaction(async transaction => {
        ...
        transaction.set(adminDb.collection('notifications').doc(), { recipientId: authUser.uid, recipientRole: 'SCHOOL', schoolId: schoolRef.id, type: 'SCHOOL_ONBOARDING_APPROVED', title: 'Your Jaystarbliss school portal is ready', message: `Your school partnership has been approved. Your school code is ${code}. Sign in through the School Portal using your administrator email and complete the password setup from the secure invitation sent by the administrator.`, read: false, priority: 'HIGH', createdAt: now });
    ...
    try { passwordSetupLink = await adminAuth.generatePasswordResetLink(email); } catch (error) { console.warn('Could not generate school password setup link:', error); }
    ...
    console.error('School partnership approval error:', error);
    if (createdUid) await adminAuth.deleteUser(createdUid).catch(() => undefined);
    return json(500, { error: error?.message || 'Unable to approve this school partnership right now.' });
  }
};
