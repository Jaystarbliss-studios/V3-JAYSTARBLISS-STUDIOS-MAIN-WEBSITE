...
export const handler: Handler = async event => {
  ...
  try {
    ...
    const collections = ['individualStudents', 'students'];
    ...
    if (isSchool) {
      const schoolId = String(caller.schoolId || '');
      if (!schoolId || String(student.schoolId || '') !== schoolId) return json(403, { error: 'You can only issue credentials for students in your school.' });
    }
    ...
    const existing = await adminDb.collection('individualStudents').where('accessCodeHash', '==', accessCodeHash).limit(1).get();
    if (!existing.empty) return json(409, { error: 'A credential collision occurred. Please try again.' });
    ...
    console.error('Student credential issuance error:', error);
    return json(500, { error: 'Unable to issue student credentials.' });
  }
};
