...
const assignedTo = (student: Record<string, any>, uid: string) => [
  student.tutorId,
  student.staffId,
...
const serialise = (id: string, data: Record<string, any>) => ({
  id,
  ...data,
...
const canAccessAssignment = async (assignment: Record<string, any>, uid: string, role: string) => {
  ...
  if (role === 'TUTOR' || role === 'STAFF' || role === 'INSTRUCTOR') return String(assignment.tutorId || assignment.staffId || '') === uid;
  const student = await findStudent(String(assignment.studentId || ''));
  if (!student) return false;
  if (role === 'STUDENT') return String(student.firebaseUid || student.userId || '') === uid || String(assignment.studentUserId || '') === uid;
...
const listAssignments = async (uid: string, role: string, queryParams: URLSearchParams) => {
  ...
  if (['TUTOR', 'STAFF', 'INSTRUCTOR'].includes(role)) {
    const [individual, students] = await Promise.all([
      adminDb.collection('individualStudents').where('tutorId', '==', uid).limit(100).get(),
      adminDb.collection('students').where('tutorId', '==', uid).limit(100).get(),
    ]);
    individual.forEach(d => studentIds.push(d.id));
    students.forEach(d => studentIds.push(d.id));
    const [individual2, students2] = await Promise.all([
      adminDb.collection('individualStudents').where('staffId', '==', uid).limit(100).get(),
      adminDb.collection('students').where('staffId', '==', uid).limit(100).get(),
    ]);
    ...
    const user = await userRecord(uid);
    const schoolId = String(user.schoolId || uid);
    const [individual, students] = await Promise.all([
  ...
  const chunks: string[][] = [];
  for (let i = 0; i < uniqueStudentIds.length; i += 10) chunks.push(uniqueStudentIds.slice(i, i + 10));
  const results: Record<string, any>[] = [];
  for (const chunk of chunks) {
    const snap = await adminDb.collection('assignments').where('studentId', 'in', chunk).limit(200).get();
    snap.forEach(d => {
      const item = serialise(d.id, d.data() as Record<string, any>);
      if (!requestedStatus || String(item.status || '') === requestedStatus) results.push(item);
    });
  }
...
export const handler: Handler = async (event) => {
  try {
    ...
    if (action === 'create') {
      ...
      const studentId = safeText(body.studentId, 200);
      ...
      const instructions = safeText(body.instructions, 8000);
      ...
      if (Number.isNaN(dueDate.getTime())) return json(400, { error: 'Choose a valid due date.' });
      if (dueDate.getTime() < Date.now() - 60000) return json(400, { error: 'Due date cannot be in the past.' });
      const tutorName = String(user.name || user.displayName || decoded.name || decoded.email || 'Tutor');
      ...
      const payload = {
        ...
        staffId: decoded.uid,
        tutorName,
        tutorEmail: decoded.email || user.email || '',
        studentId,
        studentUserId: student.firebaseUid || student.userId || null,
        studentName: student.fullName || student.studentName || student.name || 'Student',
        parentId: student.parentId || null,
        schoolId: student.schoolId || null,
        programId: safeText(body.programId, 200) || student.programId || null,
        moduleId: safeText(body.moduleId, 200) || null,
        resourceUrl: safeText(body.resourceUrl, 2000) || null,
        resourceTitle: safeText(body.resourceTitle, 200) || null,
        dueDate,
        status: 'PUBLISHED',
        grade: null,
        feedback: null,
    ...
    const assignmentId = safeText(body.assignmentId, 200);
    if (!assignmentId) return json(400, { error: 'Assignment is required.' });
    ...
    if (!(await canAccessAssignment(assignment, decoded.uid, role))) return json(403, { error: 'You are not authorized to access this assignment.' });

    if (action === 'submit') {
      if (role !== 'STUDENT') return json(403, { error: 'Only the assigned student can submit this assignment.' });
      if (['REVIEWED', 'COMPLETED'].includes(String(assignment.status || ''))) return json(409, { error: 'This assignment has already been reviewed.' });
      const submissionText = safeText(body.submissionText, 12000);
      const submissionUrl = safeText(body.submissionUrl, 4000);
    ...
    if (action === 'review') {
      ...
      if (!Number.isFinite(grade) || grade < 0 || grade > 100) return json(400, { error: 'Grade must be between 0 and 100.' });
      const feedback = safeText(body.feedback, 12000);
      if (!feedback) return json(400, { error: 'Feedback is required when reviewing an assignment.' });
      const reviewStatus = String(body.reviewStatus || 'COMPLETED').toUpperCase();
      if (!['COMPLETED', 'RESUBMISSION_REQUIRED'].includes(reviewStatus)) return json(400, { error: 'Choose completed or resubmission required.' });
    ...
    console.error('Academic assignments error:', error);
    return json(500, { error: 'Unable to complete the assignment request.' });
  }
};
