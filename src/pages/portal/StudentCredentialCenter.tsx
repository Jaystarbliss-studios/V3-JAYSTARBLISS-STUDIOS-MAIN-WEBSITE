...
import { Download, FileText, KeyRound, Loader2, RefreshCw, ShieldCheck, Users, ChevronRight } from 'lucide-react';
...
const StudentCredentialCenter: React.FC<Props> = ({ role }) => {
  ...
  const classGroups = React.useMemo(() => {
    ...
    students.forEach(student => { const key = student.class || student.grade || 'Not Assigned'; map.set(key, (map.get(key) || 0) + 1); });
    return Array.from(map.entries()).sort((a,b) => a[0].localeCompare(b[0], undefined, {numeric:true}));
  }, [students]);
  const visibleStudents = selectedClass === 'All Classes' ? students : students.filter(student => (student.class || student.grade || 'Not Assigned') === selectedClass);
