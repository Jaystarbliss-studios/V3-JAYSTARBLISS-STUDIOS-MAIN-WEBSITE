...
const SchoolDashboard: React.FC<SchoolDashboardProps> = ({ initialTab }) => {
  ...
  return <div className="space-y-6 max-w-7xl mx-auto pb-12">
    ...
    <div className="pro-surface rounded-3xl p-6 md:p-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4"><div><div className="text-xs uppercase tracking-widest font-black text-brand-red">School Operations</div><h1 className="text-2xl md:text-3xl font-black mt-1">{schoolName}</h1><p className="text-sm text-slate-500 mt-2">Manage assessments, invigilator passcodes, school links and institutional operations without exposing student portal credentials.</p></div><button type="button" onClick={() => void load()} className="min-h-11 rounded-xl border border-slate-200 px-4 text-xs font-black inline-flex items-center gap-2"><RefreshCw size={15}/> Refresh</button></div>
      <div className="flex flex-wrap gap-2 mt-5">{(['overview','roster','exams','passcodes','resources','links','schedules','partnership'] as SchoolDashboardTab[]).map(item => <button key={item} type="button" onClick={() => changeTab(item)} className={`min-h-10 rounded-xl px-3 text-xs font-black ${tab === item ? 'bg-brand-red text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200'}`}>{item[0].toUpperCase()+item.slice(1)}</button>)}</div>
    </div>
